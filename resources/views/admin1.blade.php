    ...
    @foreach($admin as $row)
      @if($row->c_report >= 40)
        <tr>       
            ...
            <td align="center" >{{ $row->c_report }}</td>
            @if( $row->status != 0)
                <td align="center"><a onclick="return JSalert()" class="btn btn-danger"  href="{{ url('banupdate', $row->id_post) }}">ระงับการเผยแพร่</a></td>          
            @else
                <td align="center"><a onclick="return JSalert2()" class="btn btn-success"  href="{{ url('rebanupdate', $row->id_post) }}">เปิดการเผยแพร่</a></td>          
            @endif
            <td align="center" ><a class="btn btn-success"  href="{{ url('post', $row->id_post) }}" target="_blank">รายละเอียด</td>
